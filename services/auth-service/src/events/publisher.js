...
async function connect() {
  ...
  try {
    ...
    conn.on('close', () => { logger.warn('RabbitMQ connection closed, will reconnect on next publish'); channel = null; });
    return channel;
  } catch (err) {
...
/**
 * Publishes a domain event. Failures are logged but never crash the
 * request path — auth succeeds even if the event bus is briefly down;
 * a production system would add an outbox/retry table for guaranteed
 * delivery, noted here as a follow-up hardening item.
 */
